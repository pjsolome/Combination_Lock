...
int redPin = 18; // Define the red pin
int greenPin = 2; // Define the green pin

char passWord[] = {"0524"}; // Save the correct password
  ...
  if (keyPressed) {
    ...
    if (keyInNum == 4) {
      ...
      if (isRight) {                  // If the input password is right
        ...
        digitalWrite(greenPin, HIGH); //Green light indicates password was correct
        ...
        digitalWrite(redPin, HIGH);// Red light indicates password was wrong
        delay(1000);
        digitalWrite(redPin, LOW);
